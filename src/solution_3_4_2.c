...
static int sol_open(struct inode * inode, struct file * filp)
{
    ...
    /* not optimized -> need to malloc mem in write method */
    sol_st->buf = kmalloc(sol_st->buf_size, GFP_KERNEL);

    /*------------------------------------------------------------------------*/

    /* in real module i haven't to do it, but here on stepik ... */
